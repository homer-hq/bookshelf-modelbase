  ...
  var model = bookshelf.Model.extend({
    constructor: function () {
      ...
      if (this.validate) {
        ...
        this.validate = Joi.isSchema(this.validate)
    ...
    validateSave: function (model, attrs, options) {
      ...
      if ((model && !model.isNew()) || (options && (options.method === 'update' || options.patch === true))) {
        var schemaKeys = this.validate.$_terms.keys.map(function (child) {
          return child.key
        })
        ...
        const schema = optionalKeys.length
          // optionalKeys() doesn't like empty arrays
          ? this.validate.fork(optionalKeys, schema => schema.optional())
          : this.validate
        validation = schema.validate(attrs)
      } else {
        validation = this.validate.validate(this.attributes)
      }
